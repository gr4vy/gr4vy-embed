...
| `compactPaymentOptions`   | `false`      | An optional boolean to render Embed in a compact variant. Collapsed payment option rows shrink from ~80px to ~56px, option labels drop from bold/18px to medium/14px, the header/body divider is removed, and the expanded accordion body uses tighter padding (~12px vs ~24px). The card form uses inline float labels inside the inputs (no separate label row) for Card number, Expiry date (MM/YY) and CVV; the stored-card CVV prompt uses the same inline pattern. The billing details form is also tightened: taller/bolder section headings become compact headings, the tax-ID row uses an inline float label, and the form keeps matching top and bottom padding. Default remains `false` — existing integrations are unchanged.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
Please note that if you both have the `form` option set and use `embed.submit()` you might notice Embed submitting twice in a row and you'll see a warning log in debug mode. This is generally not an issue, except for wallets where sessions are created twice potentially leading to errors such as Apple Pay's "Page already has an active payment session.". If you're submitting Embed without a form, you should not pass the `form` option to the `setup` function to avoid such issues.
