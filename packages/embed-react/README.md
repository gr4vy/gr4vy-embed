...
| `approvalExpiresAt` | `null` | An optional ISO 8601 datetime string that sets the expiration date for transaction approval. When set to `null` or omitted, no approval expiration is applied. Client-side validation only checks that this value is a string; the API validates the actual format. |
