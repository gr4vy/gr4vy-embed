...
project relies on the `pnpm` package manager. Additionally this is a monorepo
that depends on `pnpm` workspaces to manage multiple packages
...
> **Note:** By default `pnpm run dev` will open on http://localhost:8081/ yet you
...
> **Note:** We try to keep a coverage of a 100%. Run the `pnpm run test` command to
